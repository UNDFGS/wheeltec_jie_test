...
class MapViewerWindow(QWidget):
    ...
    def _build_simple_dog_actor(self) -> vtk.vtkAssembly | None:
        try:
            # 优先使用 wheeltec R550 Plus URDF
            try:
                urdf_path = (
                    Path(get_package_share_directory("wheeltec_robot_urdf"))
                    / "urdf" / "autoware_akm_ultra_single_antenna_robot.urdf"
                )
            except Exception:
                urdf_path = (
            ...
            self.info_label.setText(f"加载 URDF 失败：{exc}")
            return None
